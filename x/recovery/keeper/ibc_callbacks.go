...
import (
	...
	"github.com/Atrix/Atrix/v11/ibc"
...
//   - sends over all Atrix native tokens
//
...
func (k Keeper) OnRecvPacket(
	...
	// Get addresses in `Atrix1` and the original bech32 format
	...
	// a sender's own account on Atrix (sender == recipient)
	...
	if account != nil && Atrix.IsSupportedKey(account.GetPubKey()) {
		return ack
	}
	...
	k.bankKeeper.IterateAccountBalances(ctx, recipient, func(coin sdk.Coin) (stop bool) {
		...
		packetTransfer := &transfertypes.MsgTransfer{
			...
			Sender:           recipient.String(),        // recipient is the address in the Atrix chain
...
// the IBC denomination, i.e port and channel on Atrix for the voucher. It
// returns an error if:
