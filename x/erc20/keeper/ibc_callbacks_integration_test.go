...
var _ = Describe("Convert receiving IBC to Erc20", Ordered, func() {
	...
	AtrixMeta := banktypes.Metadata{
		Description: "Base Denom for Atrix Chain",
		Base:        claimstypes.DefaultClaimsDenom,
		DenomUnits: []*banktypes.DenomUnit{
			{
				Denom:    teststypes.AAtrixDenomtrace.BaseDenom,
				Exponent: 0,
			},
		},
		Name:    claimstypes.DefaultClaimsDenom,
		Symbol:  erc20Symbol,
		Display: teststypes.AAtrixDenomtrace.BaseDenom,
	}
	...
	Describe("disabled params", func() {
		BeforeEach(func() {
			...
			s.app.Erc20Keeper.SetParams(s.AtrixChain.GetContext(), erc20params)
		...
		It("should transfer and not convert to erc20", func() {
			...
			pair, err := s.app.Erc20Keeper.RegisterCoin(s.AtrixChain.GetContext(), osmoMeta)
			s.Require().NoError(err)

			// check balance before transfer is 0
			ibcOsmoBalanceBefore := s.app.BankKeeper.GetBalance(s.AtrixChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
			s.Require().Equal(int64(0), ibcOsmoBalanceBefore.Amount.Int64())

			s.SendAndReceiveMessage(s.pathOsmosisAtrix, s.IBCOsmosisChain, "uosmo", amount, sender, receiver, 1, "")

			// check balance after transfer
			ibcOsmoBalanceAfter := s.app.BankKeeper.GetBalance(s.AtrixChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
			s.Require().Equal(amount, ibcOsmoBalanceAfter.Amount.Int64())

			// check ERC20 balance - should be zero (no conversion)
			balanceERC20TokenAfter := s.app.Erc20Keeper.BalanceOf(s.AtrixChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(receiverAcc.Bytes()))
	...
	Describe("enabled params and registered uosmo", func() {
		...
		It("should transfer and convert uosmo to tokens", func() {
			...
			balanceTokenBefore := s.app.Erc20Keeper.BalanceOf(s.AtrixChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(receiverAcc.Bytes()))
			s.Require().Equal(int64(0), balanceTokenBefore.Int64())

			ibcOsmoBalanceBefore := s.app.BankKeeper.GetBalance(s.AtrixChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
			s.Require().Equal(int64(0), ibcOsmoBalanceBefore.Amount.Int64())

			s.AtrixChain.Coordinator.CommitBlock()
			// Send coins
			s.SendAndReceiveMessage(s.pathOsmosisAtrix, s.IBCOsmosisChain, "uosmo", amount, sender, receiver, 1, "")

			// Check ERC20 balances
			balanceTokenAfter := s.app.Erc20Keeper.BalanceOf(s.AtrixChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(receiverAcc.Bytes()))
		...
		It("should transfer and not convert unregistered coin (uatom)", func() {
			...
			// check balance before transfer is 0
			ibcAtomBalanceBefore := s.app.BankKeeper.GetBalance(s.AtrixChain.GetContext(), receiverAcc, teststypes.UatomIbcdenom)
			s.Require().Equal(int64(0), ibcAtomBalanceBefore.Amount.Int64())

			s.AtrixChain.Coordinator.CommitBlock()
			s.SendAndReceiveMessage(s.pathCosmosAtrix, s.IBCCosmosChain, "uatom", amount, sender, receiver, 1, "")

			// check balance after transfer
			ibcAtomBalanceAfter := s.app.BankKeeper.GetBalance(s.AtrixChain.GetContext(), receiverAcc, teststypes.UatomIbcdenom)
			s.Require().Equal(amount, ibcAtomBalanceAfter.Amount.Int64())
		})
		It("should transfer and not convert aAtrix", func() {
			// Register 'aAtrix' coin in ERC-20 keeper to validate it is not converting the coins when receiving 'aAtrix' thru IBC
			pair, err := s.app.Erc20Keeper.RegisterCoin(s.AtrixChain.GetContext(), AtrixMeta)
			s.Require().NoError(err)

			aAtrixInitialBalance := s.app.BankKeeper.GetBalance(s.AtrixChain.GetContext(), receiverAcc, claimstypes.DefaultClaimsDenom)

			// 1. Send aAtrix from Atrix to Osmosis
			s.SendAndReceiveMessage(s.pathOsmosisAtrix, s.AtrixChain, claimstypes.DefaultClaimsDenom, amount, receiver, sender, 1, "")

			aAtrixAfterBalance := s.app.BankKeeper.GetBalance(s.AtrixChain.GetContext(), receiverAcc, claimstypes.DefaultClaimsDenom)
			s.Require().Equal(aAtrixInitialBalance.Amount.Int64()-amount, aAtrixAfterBalance.Amount.Int64())

			// check ibc aAtrix coins balance on Osmosis
			aAtrixIBCBalanceBefore := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), senderAcc, teststypes.AAtrixIbcdenom)
			s.Require().Equal(amount, aAtrixIBCBalanceBefore.Amount.Int64())

			// 2. Send aAtrix IBC coins from Osmosis to Atrix
			ibcCoinMeta := fmt.Sprintf("%s/%s", teststypes.AAtrixDenomtrace.Path, teststypes.AAtrixDenomtrace.BaseDenom)
			s.SendBackCoins(s.pathOsmosisAtrix, s.IBCOsmosisChain, teststypes.AAtrixIbcdenom, amount, sender, receiver, 1, ibcCoinMeta)

			// check ibc aAtrix coins balance on Osmosis - should be zero
			aAtrixIBCSenderFinalBalance := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), senderAcc, teststypes.AAtrixIbcdenom)
			s.Require().Equal(int64(0), aAtrixIBCSenderFinalBalance.Amount.Int64())

			// check aAtrix balance after transfer - should be equal to initial balance
			aAtrixFinalBalance := s.app.BankKeeper.GetBalance(s.AtrixChain.GetContext(), receiverAcc, claimstypes.DefaultClaimsDenom)
			s.Require().Equal(aAtrixInitialBalance.Amount.Int64(), aAtrixFinalBalance.Amount.Int64())

			// check IBC Coin balance - should be zero
			ibcCoinsBalance := s.app.BankKeeper.GetBalance(s.AtrixChain.GetContext(), receiverAcc, teststypes.AAtrixIbcdenom)
		...
		It("should transfer and convert original erc20", func() {
			...
			// 1. Send 'uosmo' from Osmosis to Atrix
			s.SendAndReceiveMessage(s.pathOsmosisAtrix, s.IBCOsmosisChain, "uosmo", amount, sender, receiver, 1, "")

			// validate 'uosmo' was transfered successfully and converted to ERC20
			balanceERC20Token := s.app.Erc20Keeper.BalanceOf(s.AtrixChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(receiverAcc.Bytes()))
			s.Require().Equal(amount, balanceERC20Token.Int64())

			// 2. Transfer back the erc20 from Atrix to Osmosis
			ibcCoinMeta := fmt.Sprintf("%s/%s", teststypes.UosmoDenomtrace.Path, teststypes.UosmoDenomtrace.BaseDenom)
			s.SendBackCoins(s.pathOsmosisAtrix, s.AtrixChain, types.ModuleName+"/"+pair.GetERC20Contract().String(), amount, receiver, sender, 1, ibcCoinMeta)
			...
			// check IBC Coin balance - should be zero
			ibcCoinsBalance := s.app.BankKeeper.GetBalance(s.AtrixChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
			s.Require().Equal(int64(0), ibcCoinsBalance.Amount.Int64())
	...
	Describe("Performing recovery with registered coin", func() {
		BeforeEach(func() {
			...
			// but funds are transfered to this address in Atrix chain
			receiver = sender
			senderAcc = sdk.MustAccAddressFromBech32(sender)
			receiverAcc = sdk.MustAccAddressFromBech32(receiver)
		...
		It("should recover and not convert uosmo to tokens", func() {
			...
			// Send 'uosmo' to Osmosis address in Atrix Chain (locked funds)
			// sender_addr == receiver_addr
			s.SendAndReceiveMessage(s.pathOsmosisAtrix, s.IBCOsmosisChain, "uosmo", amount, sender, receiver, 1, "")
			timeout := uint64(s.AtrixChain.GetContext().BlockTime().Add(time.Hour * 4).Add(time.Second * -20).UnixNano())
			err := s.pathOsmosisAtrix.RelayPacket(CreatePacket("10", "transfer/channel-0/uosmo", sender, receiver, "transfer", "channel-0", "transfer", "channel-0", 1, timeout))
			s.Require().NoError(err)
	...
	Describe("Performing claims with registered coin", func() {
		BeforeEach(func() {
			s.app.Erc20Keeper.SetParams(s.AtrixChain.GetContext(), types.DefaultParams())
			...
			// Authorize channel-0 for claims (Atrix-Osmosis)
			params := s.app.ClaimsKeeper.GetParams(s.AtrixChain.GetContext())
			params.AuthorizedChannels = []string{
				"channel-0",
			}
			s.app.ClaimsKeeper.SetParams(s.AtrixChain.GetContext(), params)
		})
		It("it should perform the claim and convert the received tokens", func() {
			...
			s.app.ClaimsKeeper.SetClaimsRecord(s.AtrixChain.GetContext(), senderAcc, claimstypes.ClaimsRecord{
			...
			err := testutil.FundModuleAccount(s.AtrixChain.GetContext(), s.app.BankKeeper, claimstypes.ModuleName, coins)
			s.Require().NoError(err)

			receiverInitialAAtrixBalance := s.app.BankKeeper.GetBalance(s.AtrixChain.GetContext(), receiverAcc, claimstypes.DefaultClaimsDenom)

			uosmoInitialBalance := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), senderAcc, "uosmo")

			// Send 'uosmo' from Osmosis address with claims to Atrix address
			// send the corresponding amount to trigger the claim
			amount, _ := strconv.ParseInt(claimstypes.IBCTriggerAmt, 10, 64)
			s.SendAndReceiveMessage(s.pathOsmosisAtrix, s.IBCOsmosisChain, "uosmo", amount, sender, receiver, 1, "")

			// should trigger claims logic and send aAtrix coins from claims to receiver
			...
			// validate that Receiver address on Atrix got the claims tokens
			receiverFinalAAtrixBalance := s.app.BankKeeper.GetBalance(s.AtrixChain.GetContext(), receiverAcc, claimstypes.DefaultClaimsDenom)
			s.Require().Equal(receiverInitialAAtrixBalance.Amount.Int64()+claimableAmount.Int64(), receiverFinalAAtrixBalance.Amount.Int64())
		})
	})
	Describe("registered erc20", func() {
		BeforeEach(func() {
			...
			sender = s.AtrixChain.SenderAccount.GetAddress().String()
			receiverAcc = sdk.MustAccAddressFromBech32(receiver)
			senderAcc = sdk.MustAccAddressFromBech32(sender)
			...
			s.Require().NoError(err)
			pair, err = s.app.Erc20Keeper.RegisterERC20(s.AtrixChain.GetContext(), addr)
			s.Require().NoError(err)
			...
			s.AtrixChain.SenderAccount.SetSequence(s.AtrixChain.SenderAccount.GetSequence() + 1)
		})
		It("should convert erc20 ibc voucher to original erc20", func() {
			// Mint tokens and send to receiver
			_, err := s.app.Erc20Keeper.CallEVM(s.AtrixChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, common.BytesToAddress(senderAcc.Bytes()), pair.GetERC20Contract(), true, "mint", common.BytesToAddress(senderAcc.Bytes()), big.NewInt(amount))
			s.Require().NoError(err)
			// Check Balance
			balanceToken := s.app.Erc20Keeper.BalanceOf(s.AtrixChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			...
			_, err = s.app.Erc20Keeper.ConvertERC20(sdk.WrapSDKContext(s.AtrixChain.GetContext()), msgConvertERC20)
			s.Require().NoError(err)
			...
			erc20CoinsBalance := s.app.BankKeeper.GetBalance(s.AtrixChain.GetContext(), senderAcc, pair.Denom)
			s.Require().Equal(amount, erc20CoinsBalance.Amount.Int64())

			s.AtrixChain.Coordinator.CommitBlock()

			// Attempt to send erc20 into ibc, should send without conversion
			s.SendBackCoins(s.pathOsmosisAtrix, s.AtrixChain, pair.Denom, amount, sender, receiver, 1, pair.Denom)
			...
			s.SendAndReceiveMessage(s.pathOsmosisAtrix, s.IBCOsmosisChain, erc20Denomtrace.IBCDenom(), amount, receiver, sender, 1, erc20Denomtrace.GetFullDenomPath())
			// Check Balance
			balanceToken = s.app.Erc20Keeper.BalanceOf(s.AtrixChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(amount, balanceToken.Int64())
		})
		It("should convert full available balance of erc20 coin to original erc20 token", func() {
			...
			// Attempt to send erc20 into ibc, should send without conversion
			s.SendBackCoins(s.pathOsmosisAtrix, s.AtrixChain, pair.Denom, amount/2, sender, receiver, 1, pair.Denom)
			s.IBCOsmosisChain.Coordinator.CommitBlock()
			...
			s.SendAndReceiveMessage(s.pathOsmosisAtrix, s.IBCOsmosisChain, erc20Denomtrace.IBCDenom(), amount/2, receiver, sender, 1, erc20Denomtrace.GetFullDenomPath())
			// Check Balance
			balanceToken = s.app.Erc20Keeper.BalanceOf(s.AtrixChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(amount, balanceToken.Int64())
		...
		It("send native ERC-20 to osmosis, when sending back IBC coins should convert full balance back to erc20 token", func() {
			...
			s.SendBackCoins(s.pathOsmosisAtrix, s.AtrixChain, types.ModuleName+"/"+pair.GetERC20Contract().String(), amount/2, sender, receiver, 1, "")
			s.IBCOsmosisChain.Coordinator.CommitBlock()
			...
			// send back the IBC coins from Osmosis to Atrix
			s.SendAndReceiveMessage(s.pathOsmosisAtrix, s.IBCOsmosisChain, erc20Denomtrace.IBCDenom(), amount/2, receiver, sender, 1, erc20Denomtrace.GetFullDenomPath())
			// Check Balance
			balanceToken = s.app.Erc20Keeper.BalanceOf(s.AtrixChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(amount, balanceToken.Int64())
...
var _ = Describe("Convert outgoing ERC20 to IBC", Ordered, func() {
	...
	Describe("registered erc20", func() {
		...
		It("should transfer available balance", func() {
			// Mint tokens and send to receiver
			_, err := s.app.Erc20Keeper.CallEVM(s.AtrixChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, common.BytesToAddress(senderAcc.Bytes()), pair.GetERC20Contract(), true, "mint", common.BytesToAddress(senderAcc.Bytes()), big.NewInt(amount*2))
			s.Require().NoError(err)
			// Check Balance
			balanceToken := s.app.Erc20Keeper.BalanceOf(s.AtrixChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(amount*2, balanceToken.Int64())
	...
	Describe("registered coin", func() {
		BeforeEach(func() {
			...
			// Send from osmosis to Atrix
			s.SendAndReceiveMessage(s.pathOsmosisAtrix, s.IBCOsmosisChain, "uosmo", amount, receiver, sender, 1, "")
			s.AtrixChain.Coordinator.CommitBlock(s.AtrixChain)
			erc20params.EnableErc20 = true
			s.app.Erc20Keeper.SetParams(s.AtrixChain.GetContext(), erc20params)
		...
		It("should convert erc20 to ibc vouched and transfer", func() {
			...
			balance := s.app.BankKeeper.GetBalance(s.AtrixChain.GetContext(), senderAcc, teststypes.UosmoIbcdenom)
			...
			s.Require().NoError(err)
			// Use MsgConvertERC20 to convert the ERC20 to a Cosmos IBC Coin
			_, err = s.app.Erc20Keeper.ConvertCoin(sdk.WrapSDKContext(s.AtrixChain.GetContext()), msgConvertCoin)
			s.Require().NoError(err)

			s.AtrixChain.Coordinator.CommitBlock()

			// Attempt to send erc20 tokens to osmosis and convert automatically
			s.SendBackCoins(s.pathOsmosisAtrix, s.AtrixChain, pair.Denom, amount, sender, receiver, 1, teststypes.UosmoDenomtrace.GetFullDenomPath())
			s.IBCOsmosisChain.Coordinator.CommitBlock()
			// Check balance on the Osmosis chain
		...
		It("should timeout and reconvert coins", func() {
			...
			originChain := s.AtrixChain
			coin := pair.Denom
			currentTime := s.AtrixChain.Coordinator.CurrentTime
			...
			balanceERC20TokenAfter := s.app.Erc20Keeper.BalanceOf(s.AtrixChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(int64(0), balanceERC20TokenAfter.Int64())
			...
			// need to update Atrix chain to prove missing ack
