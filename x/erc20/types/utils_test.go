	...
	testCases := []struct {
	...
	}{
		{
			...
			banktypes.Metadata{
				...
				Name:        "Atrix",
				Symbol:      "Atrix",
				Description: "EVM, staking and governance denom of Atrix",
				DenomUnits: []*banktypes.DenomUnit{
					{
						Denom:    "aAtrix",
						Exponent: 0,
						Aliases:  []string{"atto Atrix"},
					},
					{
						Denom:    "Atrix",
						Exponent: 18,
...
func TestEqualAliases(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			[]string{"atto Atrix", "aAtrix"},
			[]string{"aAtrix", "atto Atrix"},
			false,
		},
		{
			"same values, sorted",
