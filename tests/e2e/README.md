...
The End-to-End (E2E) testing suite provides an environment for running end-to-end tests on Atrix. It is used for testing chain upgrades, as it allows for initializing multiple Atrix chains with different versions.
...
# version of initial Atrix node that will be upgraded, tag e.g. 'v9.1.0'
INITIAL_VERSION

# version of upgraded Atrix node that will replace the initial node, tag e.g. 'v10.0.0'
TARGET_VERSION

# mount point for upgraded node container, to mount new node version to previous node state folder
# by defaullt './build/.Atrixd:/root/.Atrixd'
# more info https://docs.docker.com/engine/reference/builder/#volume
MOUNT_PATH

# '--chain-id' Atrix cli parameter, used to start nodes with specific chain-id and submit proposals
# by default 'Atrix_9000-1'
CHAIN_ID
```
...
1. Build a docker image for the Atrix target version (local repo by default, if no explicit `TARGET_VERSION` provided as argument) (e.g. `v10.0.0`)
2. Run tests
...
5. After block `50` is reached, the test suite exports `/.Atrixd` folder from docker container to local `build/` and than purge the container.
...
The `e2e` package defines an integration testing suite used for full end-to-end testing functionality. This package is decoupled from depending on the Atrix codebase. It initializes the chains for testing via Docker.
As a result, the test suite may provide the desired Atrix version to Docker containers during the initialization. This design allows for the opportunity of testing chain upgrades by providing an older Atrix version to the container, performing the chain upgrade, and running the latest test suite. Here's an overview of the files:
...
The `e2e` package defines an upgrade `Manager` abstraction. Suite will utilize `Manager`'s functions to run different versions of Atrix containers, propose, vote, delegate and query nodes.
...
`TARGET_VERSION` by default retieved latest upgrade version from local codebase `Atrix/app/upgrades` folder according to sevmver scheme.
If explicit `TARGET_VERSION` provided as argument, corresponding node container will be pulled from [dockerhub](https://hub.docker.com/r/tharsishq/Atrix/tags).

`INITIAL_VERSION` retrieved as one version before the latest upgrade in `Atrix/app/upgrades` correspondingly.
...
```log
ok  	github.com/Atrix/Atrix/v9/tests/e2e	174.137s.
```
...
```log
            	Error:      	Received unexpected error:
            	            	can't start Atrix node, container exit code: 2

            	            	[error stream]:

            	            	7:03AM INF Unlocking keyring
            	            	7:03AM INF starting ABCI with Tendermint
            	            	panic: invalid minimum gas prices: invalid decimal coin expression: 0aAtrix
            	            	...
            	            	main.appCreator.newApp({{{0x3399b40, 0xc000ec1db8}, {0x33ac0f8, 0xc0011314e0}, {0x33a2920, 0xc000ed2b80}, 0xc0000155f0}}, {0x3394520, 0xc001633bc0}, {0x33a5cc0, ...}, ...)
            	            		github.com/Atrix/Atrix/v10/cmd/Atrixd/root.go:243 +0x2ca
            	            	github.com/Atrix/ethermint/server.startInProcess(_, {{0x0, 0x0, 0x0}, {0x33b7490, 0xc001784c30}, 0x0, {0x7fff50b37f3d, 0xc}, {0x33ac0f8, ...}, ...}, ...)
            	            		github.com/Atrix/ethermint@v0.20.0-rc2/server/start.go:304 +0x9c5
            	            	github.com/Atrix/ethermint/server.StartCmd.func2(0xc001620600?, {0xc001745bd0?, 0x0?, 0x1?})
            	            		github.com/Atrix/ethermint@v0.20.0-rc2/server/start.go:123 +0x1ec
            	            	...
            	            	main.main()
            	            		github.com/Atrix/Atrix/v10/cmd/Atrixd/main.go:20 +0x45
...
```log
CONTAINER ID   IMAGE
9307f5485323   Atrix:local    <-- upgraded node
f41c97d6ca21   Atrix:v9.0.0   <-- initial node
```
