...
Learn how to deploy a simple Solidity-based smart contract to Atrix using the Truffle environment {synopsis}
...
This will allow your contract to connect to your Atrix local node.
...
You should see incoming deployment logs in the Atrix daemon Terminal tab for each transaction (one to deploy `Migrations.sol` and the other to deploy `Counter.sol`).
...
Now, you can run the Truffle tests using the Atrix node using the `test` command:
