...
Learn how to register interoperable ERC-20s through [Atrix Governance](../../users/governance/overview.md). {synopsis}
...
Application-wise, the ERC-20 module allows DeFi protocols to seamlessly integrate with Atrix and the Cosmos ecosystem. Using the module, developers can build smart contracts on Atrix and use the generated ERC-20 tokens for other [applications on the Cosmos ecosystem](https://mapofzones.com), such as:
...
More information can be found in [this blog post](https://medium.com/Atrix/introducing-Atrix-erc20-module-f40a61e05273), which introduced the ERC-20 Module on Atrix.
...
2. Promote understanding of the ERC-20 Module.
   ...
   Make sure to mention the original [blog post](https://medium.com/Atrix/introducing-Atrix-erc20-module-f40a61e05273) that introduced the ERC-20 Module, along with a brief explanation of what the ERC-20 Module does. It's also a good idea to link the [ERC-20 Module documentation](https://docs.Atrix.org/modules/erc20/)!
...
4. Document adherence to ERC-20 Contract expectations.
   ...
   Ensure that the ERC-20 contracts are verified (either through the [EVM explorer](https://evm.Atrix.org) or via [Sourcify](https://sourcify.dev)), and that the contracts are deployed open-source.
...
To [submit the proposal](../../users/governance/submitting.md) to testnet through the command line with [`Atrixd`](../../validators/quickstart/binary.md), use the following command with `register-erc20`:

```bash
Atrixd tx gov submit-legacy-proposal register-erc20 ERC20_ADDRESS...\
  --title=<title> \
...
```bash
Atrixd tx gov submit-legacy-proposal register-erc20 --proposal=<path/to/proposal.json>
```
