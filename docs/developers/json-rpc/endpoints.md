...
Check the JSON-RPC methods supported on Atrix. {synopsis}
...
```json
 {"jsonrpc":"2.0","id":1,"result":"Atrix/0.1.3+/linux/go1.18"}
```
...
Make sure your `Atrixd start` call is not using the flag `minimum-gas-prices` because this value will be used instead of the one set on the configuration file.
