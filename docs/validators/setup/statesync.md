...
```bash
SNAP_RPC1="http://bd-Atrix-testnet-state-sync-node-01.bdnodes.net:26657"
SNAP_RPC="http://bd-Atrix-testnet-state-sync-node-02.bdnodes.net:26657"
CHAIN_ID="Atrix_9000-4"
PEER="3a6b22e1569d9f85e9e97d1d204a1c457d860926@bd-Atrix-testnet-seed-node-01.bdnodes.net:26656"
wget -O $HOME/genesis.json https://archive.Atrix.dev/Atrix_9000-4/genesis.json 
```

## Use commands below for Mainnet setup

```bash
SNAP_RPC1="http://bd-Atrix-mainnet-state-sync-us-01.bdnodes.net:26657"
SNAP_RPC="http://bd-Atrix-mainnet-state-sync-eu-01.bdnodes.net:26657"
CHAIN_ID="Atrix_9001-2"
PEER="96557e26aabf3b23e8ff5282d03196892a7776fc@bd-Atrix-mainnet-state-sync-us-01.bdnodes.net,dec587d55ff38827ebc6312cedda6085c59683b6@bd-Atrix-mainnet-state-sync-eu-01.bdnodes.net"
wget -O $HOME/genesis.json https://archive.Atrix.org/mainnet/genesis.json 
```
...
```bash
Atrixd init $moniker --chain-id $CHAIN_ID
```

Move genesis file to .Atrixd/config folder

```bash
mv $HOME/genesis.json ~/.Atrixd/config/
```
...
```bash
sed -i -e "s%^moniker *=.*%moniker = \"$moniker\"%; " $HOME/.Atrixd/config/config.toml
sed -i -e "s%^indexer *=.*%indexer = \"null\"%; " $HOME/.Atrixd/config/config.toml
sed -i -e "s%^persistent_peers *=.*%persistent_peers = \"$PEER\"%; " $HOME/.Atrixd/config/config.toml
```
...
s|^(seeds[[:space:]]+=[[:space:]]+).*$|\1\"\"|" ~/.Atrixd/config/config.toml
```

### Create Atrixd service

```bash
echo "[Unit]
Description=Atrixd Node
After=network.target
#
[Service]
User=$USER
Type=simple
ExecStart=$(which Atrixd) start
Restart=on-failure
LimitNOFILE=65535
#
[Install]
WantedBy=multi-user.target" > $HOME/Atrixd.service; sudo mv $HOME/Atrixd.service /etc/systemd/system/
```

```bash
sudo systemctl enable Atrixd.service && sudo systemctl daemon-reload
```

### Run Atrixd

```bash
sytemctl start Atrixd
```

### Check logs

```bash
journalctl -u Atrixd -f
```
...
```bash
sed -i.bak -E "s|^(enable[[:space:]]+=[[:space:]]+).*$|\1false|" $HOME/.Atrixd/config/config.toml
```
