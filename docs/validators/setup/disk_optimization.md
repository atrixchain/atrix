...
speed and transaction amount. For Atrix, we are talking about close to
...
I believe this was disabled by default on Atrix, but listing it in any
...
will risk database corruption if the `Atrixd` is killed for any reason.
...
Below is the disk usage after two weeks of Atrix Arsia Mons testnet. The default
