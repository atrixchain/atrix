...
`Atrixd` is the all-in-one command-line interface. It supports wallet management, queries and transaction operations {synopsis}
...
After you have obtained the latest `Atrixd` binary, run:
...
::: tip
You can also enable auto-completion with the `Atrixd completion` command. For example, at the start of a bash session, run `. <(Atrixd completion)`, and all `Atrixd` subcommands will be auto-completed.
:::

### Config and data directory

By default, your config and data are stored in the folder located at the `~/.Atrixd` directory.
...
To specify the `Atrixd` config and data storage directory; you can update it using the global flag `--home <directory>`

### Configuring the Node

The Cosmos SDK automatically generates two configuration files inside `~/.Atrixd/config`:
...
If you would like to change your nodes pruning strategy then you must do so when the node is initialized. Passing a flag when starting `Atrix` will always override settings in the `app.toml` file, if you would like to change your node to the `everything` mode then you can pass the `---pruning everything` flag when you call `Atrixd start`.
...
We can view the default client config setting by using `Atrixd config` command:
...
For example, the chain identifier can be changed to `Atrix_9000-4` from a blank name by using:
...
Alternatively, we can directly make the changes to the config values in one place at client.toml. It is under the path of `.Atrix/config/client.toml` in the folder where we installed Atrix:
...
After the necessary changes are made in the `client.toml`, then save. For example, if we directly change the chain-id from `Atrix_{{ $themeConfig.project.testnet_chain_id }}-2` to `Atrixtest_9000-1`, and output to number, it would change instantly as shown below.

```bash
Atrixd config
{
 "chain-id": "Atrixtest_9000-1",
 "keyring-backend": "os",
 "output": "number",
 "node": "tcp://localhost:26657",
 "broadcast-mode": "sync"
...
A list of commonly used flags of `Atrixd` is listed below:

| Option              | Description                   | Type                                             | Default Value |
| ------------------- | ----------------------------- | ------------------------------------------------ | ------------- |
| `--chain-id`        | Full Chain ID                 | `string`                                         | `""`          |
| `--home`            | Directory for config and data | `string`                                         | `~/.Atrixd`   |
| `--keyring-backend` | Select keyring's backend      | `{"os"|"file"|"kwallet"|"pass"|"test"|"memory"}` | `"os"`        |
| `--output`          | Output format                 | `string`                                         | "text"        |

## Command list

A list of commonly used `Atrixd` commands. You can obtain the full list by using the `Atrixd -h` command.
