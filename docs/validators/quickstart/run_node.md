...
Configure and run an Atrix node {synopsis}

## Pre-requisite Readings

- [Installation](./installation.md) {prereq}
- [`Atrixd`](./binary.md) {prereq}
...
```bash
Atrixd start --json-rpc.enable=true --json-rpc.api="eth,web3,net"
```

## Key Management

To run a node with the same key every time: replace `Atrixd keys add $KEY` in `./local_node.sh` with:

```bash
echo "your mnemonic here" | Atrixd keys add $KEY --recover
```

::: tip
Atrix currently only supports 24 word mnemonics.
:::
...
To export your Atrix key as an Ethereum private key (for use with [Metamask](./../../users/wallets/metamask.md) for example):

```bash
Atrixd keys unsafe-export-eth-key $KEY
```
...
Atrix supports using a file or OS keyring backend for key storage. To create and use a file
stored key instead of defaulting to the OS keyring, add the flag `--keyring-backend file` to any
relevant command and the password prompt will occur through the command line. This can also be saved
as a CLI config option with:

```bash
Atrixd config keyring-backend file
```
...
```bash
Atrixd start --evm.tracer $TRACER --tracestore $TRACESTORE --pruning=nothing $TRACE --log_level $LOGLEVEL --minimum-gas-prices=0.0001aAtrix --json-rpc.api eth,txpool,personal,net,debug,web3
```
...
::: danger
If you are running a **validator node**, always be careful when doing `Atrixd unsafe-reset-all`. You should never use this command if you are not switching `chain-id`.
:::
...
```shell
rate(Atrixd_transactions_processed[1m])
```
...
Learn about running a Atrix [testnet](./../testnet.md) {hide}
