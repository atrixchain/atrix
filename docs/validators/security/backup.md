...
```bash
~/.Atrixd/config/priv_validator_key.json
```

Then do the following:

1. Backup the `json` file mentioned above (or backup the whole `config` folder).
2. Backup the self-delegator wallet. See [backing up wallets with the Atrix Daemon](../../users/wallets/backup.md).
...
```bash
Atrixd tendermint show-address
```
