...
func (suite *UpgradeTestSuite) TestMigrateEscrowAcc() {
	suite.SetupTest(Atrixtypes.MainnetChainID)
...
func (suite *UpgradeTestSuite) TestDistributeRewards() {
	// define constants
	mainnetChainID := Atrixtypes.MainnetChainID + "-4"
	communityPool := sdk.MustAccAddressFromBech32("Atrix1jv65s3grqf6v6jl3dp4t6c9t9rk99cd8974jnh")
	...
	noRewardAddr := sdk.MustAccAddressFromBech32("Atrix1009egsf8sk3puq3aynt8eymmcqnneezkkvceav")
		...
		},
		{
			...
			func() {
				err := suite.app.BankKeeper.SendCoins(
					...
					sdk.NewCoins(
						sdk.NewCoin(Atrixtypes.BaseDenom, balance.Quo(math.NewInt(2))),
					),
				)
				suite.NoError(err)
			},
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			if Atrixtypes.IsMainnet(tc.chainID) {
			...
			// balance should be 0
			balance := suite.app.BankKeeper.GetBalance(suite.ctx, noRewardAddr, Atrixtypes.BaseDenom)
			...
			commPoolFinalBalance := suite.app.BankKeeper.GetBalance(suite.ctx, communityPool, Atrixtypes.BaseDenom)
			...
			for i := range v11.Allocations {
				...
				balance := suite.app.BankKeeper.GetBalance(suite.ctx, addr, Atrixtypes.BaseDenom)
				suite.Require().Equal(math.ZeroInt(), balance.Amount)
			...
			if tc.expectedSuccess {
				...
				finalFundingAccBalance := suite.app.BankKeeper.GetBalance(suite.ctx, fundingAcc, Atrixtypes.BaseDenom)
...
func (suite *UpgradeTestSuite) fundTestnetRewardsAcc(amount math.Int) {
	...
	rewards := sdk.NewCoins(sdk.NewCoin(Atrixtypes.BaseDenom, amount))
