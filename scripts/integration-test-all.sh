...
## default port prefixes for Atrixd
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/Atrixd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/Atrixd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/Atrixd add-genesis-account \
    "$("$PWD"/build/Atrixd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000aAtrix,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/Atrixd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/Atrixd collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/Atrixd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/Atrixd start --pruning=nothing --rpc.unsafe \
    ...
    Atrix_PID=$!
    echo "started Atrix node, pid=$Atrix_PID"
    # add PID to array
    arr+=("$Atrix_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test Atrix node $HOST_RPC ..."
...
stop_func() {
    Atrix_PID=$i
    echo "shutting down node, pid=$Atrix_PID ..."
